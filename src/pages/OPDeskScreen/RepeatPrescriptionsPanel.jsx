        ...
        {filtered.map((p, index) => (
          <div key={p.id} className="flex border-b" style={{ borderColor: "var(--color-border)", background: index % 2 === 0 ? "var(--color-surface)" : "var(--color-surface-alt)" }}>
            ...
            <div className="w-40 px-3 py-2.5 flex items-center justify-center gap-1.5">
              <button onClick={() => handleView(p)} className="p-1.5 rounded transition-all" title="View"
                style={{ background: "var(--color-primary-muted)", color: "var(--color-primary)" }}
                onMouseEnter={e => e.currentTarget.style.background = "var(--color-primary-light)"}
                onMouseLeave={e => e.currentTarget.style.background = "var(--color-primary-muted)"}>
                <Eye size={12} />
              </button>
              <button onClick={() => setModalMode({ id: p.id, title: p.title })} className="p-1.5 rounded transition-all" title="Modify"
              ...
              <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded transition-all" title="Delete"
