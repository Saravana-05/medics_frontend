  ...
  return (
    <>
      ...
      <div className="table-header-text flex items-center border-b flex-shrink-0" style={{ background: "var(--color-primary-muted)", borderColor: "var(--color-border)", height: "34px", boxSizing: "border-box" }}>
        ...
        <div className="side-panel-actions-header w-40 pl-3 pr-2" style={{ fontSize: "0.65rem", fontWeight: "800", letterSpacing: "0.03em", lineHeight: 1, color: "var(--color-primary-dark)" }}>Actions</div>
      </div>
        ...
        {filtered.map((e, index) => {
          ...
          <div key={e.id} tabIndex={0} onClick={() => setSelectedId(e.id)} onFocus={() => setSelectedId(e.id)}
            ...
            {metricsFor(e).map((m, i) => (
              <div key={i} className="w-16 px-3 py-2.5 text-center"><span className="text-xs font-normal" style={{ color: "var(--color-text-base)" }}>{m}</span></div>
            ))}
...
function EntryMirrorPanel({ sidePanel, accentColor, accentLight, accentText = "white", textAccent, icon, mirrorItems = [] }) {
  ...
  const handleLoad = item => alert(`Loaded: ${item.display?.primaryLine || item.entryType || "Entry"}`);
  const handleEdit = item => alert(`Edit: ${item.display?.primaryLine || item.entryType || "Entry"}`);
  const handleDelete = item => alert(`Delete: ${item.display?.primaryLine || item.entryType || "Entry"}`);
      ...
      <div className="ip-mirror-header table-header-text flex items-center border-b flex-shrink-0" style={{ background: "var(--color-primary-muted)", borderColor: "var(--color-border)", height: "34px", boxSizing: "border-box" }}>
        ...
        {sidePanel.columns.map((col, i) => (
          <div key={col} className={i !== 1 ? "w-24 px-3 text-center" : "flex-1 px-3 text-center"}
            style={{ fontSize: "0.65rem", fontWeight: "800", letterSpacing: "0.03em", lineHeight: 1, color: "var(--color-primary-dark)" }}>{i === 0 ? "Entry" : col}</div>
        ))}
        <div className="side-panel-actions-header ip-mirror-actions w-28 px-3" style={{ fontSize: "0.65rem", fontWeight: "800", letterSpacing: "0.03em", lineHeight: 1, color: "var(--color-primary-dark)" }}>Actions</div>
      </div>
      ...
      <div ref={rowsScrollRef} className="overflow-y-auto flex-1 no-scrollbar">
        {filtered.map((item, index) => (
          <div key={item.id} tabIndex={0} onClick={() => setSelectedId(item.id)} onFocus={() => setSelectedId(item.id)}
            ...
            <div className="ip-mirror-actions w-28 px-3 py-2.5 flex items-center justify-center gap-1.5">
              <button onClick={() => handleLoad(item)} className="p-1 rounded transition-all" title="Load" style={{ background: accentLight, color: textAccent || accentColor }}><ArrowRightCircle size={11} /></button>
              ...
              <button onClick={() => handleEdit(item)} className="p-1 rounded transition-all" title="Edit" style={{ background: accentLight, color: textAccent || accentColor }}><Edit2 size={11} /></button>
              <button onClick={() => handleDelete(item)} className="p-1 rounded transition-all" title="Delete" style={{ background: "#fee2e2", color: "var(--color-danger)" }}><X size={11} /></button>
            </div>
          </div>
        ))}
...
function FileManagerPanel({ sidePanel, accentColor, accentLight, accentText = "white", textAccent, icon }) {
  ...
  const handleViewFile = file => alert(`File Name: ${file.fileName}\nType: ${file.type}\nSize: ${file.size}\nDate-Time: ${file.dtTime}`);
  const handleLoadFile = file => alert(`Loaded: ${file.fileName}`);
  const handleEditFile = file => {
    const nextName = window.prompt("Modify file name", file.fileName);
    if (nextName?.trim()) setFiles(prev => prev.map(item => item.id === file.id ? { ...item, fileName: nextName.trim() } : item));
  };
  ...
  return (
    <>
      ...
      <div className="table-header-text flex items-center border-b flex-shrink-0" style={{ background: "var(--color-primary-muted)", borderColor: "var(--color-border)", height: "34px", boxSizing: "border-box" }}>
        ...
        <div className="side-panel-actions-header w-28 px-3" style={{ fontSize: "0.65rem", fontWeight: "800", letterSpacing: "0.03em", lineHeight: 1, color: "var(--color-primary-dark)" }}>Actions</div>
      </div>
      ...
      <div ref={rowsScrollRef} className="overflow-y-auto flex-1 no-scrollbar">
        {filtered.map((f, index) => (
          <div key={f.id} tabIndex={0} onClick={() => setSelectedId(f.id)} onFocus={() => setSelectedId(f.id)}
            ...
            <div className="w-16 px-3 py-2.5 text-center"><span className="text-xs font-normal" style={{ color: "var(--color-text-base)" }}>{f.size}</span></div>
            ...
            <div className="w-28 px-3 py-2.5 flex items-center justify-center gap-1.5">
              <button onClick={event => { event.stopPropagation(); handleLoadFile(f); }} className="p-1 rounded transition-all" title="Load" style={{ background: accentLight, color: textAccent || accentColor }}><ArrowRightCircle size={11} /></button>
              <button onClick={event => { event.stopPropagation(); handleViewFile(f); }} className="p-1 rounded transition-all" title="View" style={{ background: "var(--color-primary-muted)", color: "var(--color-primary)" }}><Eye size={11} /></button>
              <button onClick={event => { event.stopPropagation(); handleEditFile(f); }} className="p-1 rounded transition-all" title="Edit" style={{ background: accentLight, color: textAccent || accentColor }}><Edit2 size={11} /></button>
              <button onClick={event => { event.stopPropagation(); handleDelete(f.id); }} className="p-1 rounded transition-all" title="Delete" style={{ background: "#fee2e2", color: "var(--color-danger)" }}><X size={11} /></button>
            </div>
          </div>
        ))}
  ...
  return (
    <>
      ...
      <div ref={rowsScrollRef} className="overflow-y-auto flex-1 no-scrollbar">
        {filtered.map((e, index) => (
          <div key={e.id} tabIndex={0} onClick={() => setSelectedId(e.id)} onFocus={() => setSelectedId(e.id)}
            className="care-plan-panel-data-row side-panel-data-row flex items-center border-b outline-none cursor-pointer" style={{ borderColor: "var(--color-border)", background: selectedId === e.id ? accentLight : index % 2 === 0 ? "var(--color-surface)" : "var(--color-surface-alt)", boxShadow: selectedId === e.id ? `inset 0 0 0 1px ${accentColor}` : "none", height: `${ROW_HEIGHT_PX}px`, boxSizing: "border-box" }}>
            <div className="w-12 px-3 py-2.5 text-left"><span className="text-xs font-normal" style={{ color: "var(--color-text-base)" }}>{index + 1}</span></div>
            {visibleColumns.speciality && <div className="w-24 px-3 py-1 min-w-0"><span className="text-xs font-normal truncate block" style={{ color: "var(--color-text-base)" }} title={e.speciality}>{e.speciality}</span></div>}
            {visibleColumns.condition && <div className="flex-1 px-3 py-1 min-w-0"><span className="text-xs font-normal truncate block" style={{ color: "var(--color-text-base)" }} title={e.condition}>{e.condition}</span></div>}
            ...
            {visibleColumns.period && <div className="w-20 px-3 py-2.5 text-center"><span className="font-normal whitespace-nowrap" style={{ color: "var(--color-text-base)", fontSize: "calc(0.8rem - 1px)" }}>{e.period}</span></div>}
