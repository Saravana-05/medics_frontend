...
import { Search, Bell, ChevronDown, Menu, X, UserRound, Settings, LogOut, Cross, Stethoscope } from "lucide-react";
import NavDropdown from "./NavDropdown";
import navMenus from "../../../data/navMenus.json";

export default function TopNav({ user, notificationCount = 0, onLogout }) {
    ...
    <div className="flex-shrink-0 bg-white border-b border-slate-100">
      ...
      <div className="flex items-center justify-between gap-3 px-4 sm:px-6 py-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          <Link to="/opdesk" className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-semibold text-white transition-all hover:shadow-md"
            style={{ background: "linear-gradient(135deg, #2563eb, #0ea5e9)" }}>
            <Stethoscope size={15} /> <span className="hidden sm:inline">OP Desk</span>
          </Link>
